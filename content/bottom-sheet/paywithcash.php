	...
	<table class="form-container">
		...
		<tr>
			<td>
				...
				<input type="number" name="amount" placeholder="Amount" value="<?php echo (($a["Installment"]-$a["Credit"]) > 0) ? $a["Installment"]-$a["Credit"] : '0'; ?>">
			</td>
		</tr>
		<tr>
			<td>
...
$(document).ready(function() {
	$("#btnAdd").click(function() {
		...
		$.ajax({
			type: "post",
			cache: true,
			url: "process.php?action=paywithcash",
			data: {id: '<?php echo $bs_id; ?>', amount: $amount, month: '<?php echo $applicablemonth; ?>', date: $date},
